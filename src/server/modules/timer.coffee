...
class Timer extends Module
    ...
    cmdTimerStart: (user, args, bot, network) =>
        unless args? and args[0]?
            return @bot.say "[Timer] " + @str('err-invalid-name') + '. ' + @str('err-usage', '!timer <timer name>'), network
            
        name = args[0]
        @timers.add name, Date.now()
        @bot.say "[Timer] " + @str('action-timer-started', name, '!timer stop ' + name), network
        
        
    cmdTimerStop: (user, args, network) =>
        unless args? and (timer = @timers.get args[0])?
            return @bot.say "[Timer] " + @str('err-timer') + '. ' + @str('err-usage', '!timer stop <timer name>'), network
            
        @bot.say "[Timer] #{args[0]}: #{tz.timeToFullStr (Date.now() - timer)}", network
        @timers.remove args[0]
        
        
    cmdCountdownStart: (user, args, bot, network) =>
        unless args? and args[0]? and args[1]?
            return @bot.say "[Countdown] " + @str('err-invalid-name') + '. ' + @str('err-usage', '!countdown <name> <target>'), network
        
        name   = args.shift()
        target = tz.strToTime args.join('')
        @countdowns.add name, Date.now() + target
        @bot.say "[Countdown] " + @str('action-countdown-started', name, '!countdown stop ' + name), network
        

    cmdCountdownStop: (user, args, bot, network) =>
        unless args? and (timer = @countdowns.get args[0])?
            return @bot.say "[Countdown] " + @str('err-countdown') + '. ' + @str('err-usage', '!countdown stop <name>'), network
        
        @bot.say "[Countdown] " + @str('action-countdown-stopped', args[0], tz.timeToFullStr (timer - Date.now())), network
